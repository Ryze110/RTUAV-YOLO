...
def bbox_iou(
    ...
    Args:
        ...
        MPDWIoU (bool, optional): If True, calculate Minimum Point Distance Wise IoU for small target detection.
        eps (float, optional): A small value to avoid division by zero.

    Returns:
        (torch.Tensor): IoU, GIoU, DIoU, CIoU, or MPDWIoU values depending on the specified flags.
    """
    # Get the coordinates of bounding boxes
    ...
    # MPDWIoU: Minimum Point Distance Wise IoU for small target detection
    if MPDWIoU:
        # 1. 计算最小点距离 (MPD) - 基于MPDIoU论文
        # 四个角点的最小距离
        d1 = torch.sqrt((b1_x1 - b2_x1).pow(2) + (b1_y1 - b2_y1).pow(2))  # 左上角
        d2 = torch.sqrt((b1_x2 - b2_x2).pow(2) + (b1_y1 - b2_y1).pow(2))  # 右上角
        d3 = torch.sqrt((b1_x1 - b2_x1).pow(2) + (b1_y2 - b2_y2).pow(2))  # 左下角
        d4 = torch.sqrt((b1_x2 - b2_x2).pow(2) + (b1_y2 - b2_y2).pow(2))  # 右下角
        
        # 最小点距离
        mpd = torch.minimum(torch.minimum(d1, d2), torch.minimum(d3, d4))
        ...
        # 3. MPD项：最小点距离惩罚
        mpd_term = mpd.pow(2) / c2
        mpd_term = mpd_term.clamp(max=10.0)  # 防止梯度爆炸
        
        # 4. 宽高比一致性 (基于CIoU)
        v = (4 / (torch.pi ** 2)) * ((torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2))
        # denom = v - iou + 1 + eps
        # alpha = v / denom.clamp(min=eps)
        with torch.no_grad():
            alpha = v / (v - iou + (1 + eps))
        
        # 5. 小目标自适应权重 (基于WIoU的动态聚焦机制)
        area1, area2 = w1 * h1, w2 * h2
        min_area = torch.minimum(area1, area2)
        
        # 动态聚焦因子：小目标获得更高关注
        # 使用非单调函数，避免过度惩罚高质量样本
        beta = min_area / (min_area + 1024)  # 32x32 = 1024 作为小目标阈值
        gamma = 2 - 2 * iou  # 质量越高，gamma越小
        focal_weight = beta * torch.exp(-gamma)
        
        # 6. Inner-IoU机制：使用缩放的辅助框
        # 对于小目标，使用稍微放大的框来增强敏感性
        scale_factor = 1.25  # 轻微放大25%
        aux_w1, aux_h1 = w1 * scale_factor, h1 * scale_factor
        aux_w2, aux_h2 = w2 * scale_factor, h2 * scale_factor
        
        # 重新计算辅助框的坐标
        aux_w1_, aux_h1_ = aux_w1 / 2, aux_h1 / 2
        aux_w2_, aux_h2_ = aux_w2 / 2, aux_h2 / 2
        
        center1_x, center1_y = (b1_x1 + b1_x2) / 2, (b1_y1 + b1_y2) / 2
        center2_x, center2_y = (b2_x1 + b2_x2) / 2, (b2_y1 + b2_y2) / 2
        aux_b1_x1 = center1_x - aux_w1_
        aux_b1_x2 = center1_x + aux_w1_
        aux_b1_y1 = center1_y - aux_h1_
        aux_b1_y2 = center1_y + aux_h1_
        aux_b2_x1 = center2_x - aux_w2_
        aux_b2_x2 = center2_x + aux_w2_
        aux_b2_y1 = center2_y - aux_h2_
        aux_b2_y2 = center2_y + aux_h2_

        
        # 辅助框的交集和并集
        aux_inter = (aux_b1_x2.minimum(aux_b2_x2) - aux_b1_x1.maximum(aux_b2_x1)).clamp_(0) * (
            aux_b1_y2.minimum(aux_b2_y2) - aux_b1_y1.maximum(aux_b2_y1)
        ).clamp_(0)
        aux_union = aux_w1 * aux_h1 + aux_w2 * aux_h2 - aux_inter + eps
        aux_iou = aux_inter / aux_union
        
        # 7. 最终的MPDWIoU计算
        # 结合原始IoU、MPD惩罚、宽高比一致性、动态聚焦权重和辅助IoU
        mpdwiou = iou - mpd_term - alpha * v + focal_weight * (aux_iou - iou)
